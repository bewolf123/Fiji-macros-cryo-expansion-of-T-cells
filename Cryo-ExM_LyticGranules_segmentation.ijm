// Granule segmentation  and mesurement using trainable Weka 3D segmentation tr
// This macro is design to segment Perforin and GranzymeB lytic granules in 3D using pre-trained Labkit models
// It allows to get cell and synapse dimensions, granule shape, position and fluorescence intensity.
...
//Set gel size for expension factor correction
Dialog.create("Set your scale bare");
Dialog.addNumber("Gel size (cm):", 5);
Dialog.show();
gel_size = Dialog.getNumber();
ExpensionFactor = gel_size/1.2;
...
for (roi=0; roi<lengthOf(roi_name); roi++){
	roi_serie[roi] = substring(roi_name[roi], lengthOf(roi_name[roi])-13, lengthOf(roi_name[roi])-10);
...
//------------------ GET CELL SIZE AND SYNAPSE PLANE ----------------------------------------------------------------
					...
					if (method=="manual") { // The user choose the planes by hand
								...
								DomeToTop_size = (Z2-Z1)*voxel_depth;
...
//--------------------------Found the Z slice that has the highest signal in order to reset minAndMax at this specific Z.
							...
							selectImage("Probability_map_"+staining[channel]);
		  					saveAs("Tiff", dir_probability + LifName+"_serie"+cell_ID+staining[channel]+"_Probability_map.tif");
...
//------------------------- Get the ROI of each granules using 3D manager
							...
							selectWindow("Objects map of MASK_Probability_map_"+staining[channel]);
		  					saveAs("Tiff", dir_Objectmap + LifName+"_serie"+cell_ID+staining[channel]+"_object_map.tif");
...
//------------------------- Start object measurment
							...
							Ext.Manager3D_SaveResult("M",dir_result + LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv");//"M" for measure window
							Ext.Manager3D_CloseResult("M"+LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv");
							
//--------------------------Include Polarization index of each granules					
							open(dir_result + "M_"+LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv");
							
							//Rename the table to "Result" to add measures 
							Table.rename("M_"+LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv", "Results");
							selectWindow("Results");
							...
							if (n_granuls==0){
								...
								for (granuls = 0; granuls < n_granuls; granuls++) {
									...
									// Calculate the distance between the granuls and the first synapse plan (coverslip)
									GranulToSynapse = (granul_plane-Z0)*voxel_depth;
									
									// Calculate the distance of the granuls with the synaptic dome
									GranulTodome = (granul_plane-Z1)*voxel_depth;
									...
									Table.set("Synapse_size", granuls, Synapse_size);
									Table.set("Dome_to_top_size", granuls, DomeToTop_size);
									Table.set("Distance_to_synapse", granuls, GranulToSynapse);
									Table.set("Distance_to_dome", granuls, GranulTodome);
									Table.set("Polarization_index", granuls, GranulToSynapse/CellSize);
									Table.set("Polarization_index_dome", granuls, GranulTodome/DomeToTop_size);
									Table.update;
								}								
								
								//Rename the table and save the data
								Table.rename("Results", "M_"+LifName+"_serie"+cell_ID+staining[channel]+"_measure" );
								saveAs("Results", dir_result+"M_"+LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv");
								close("M_"+LifName+"_serie"+cell_ID+staining[channel]+"_measure.csv");
							}							
...
//--------------------------Select the lng image to quantify fluorescence intensity of each granules
							...
							Ext.Manager3D_SaveResult("Q",dir_result + LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv");//"Q" for quantification window
							Ext.Manager3D_CloseResult("Q"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv");
							Ext.Manager3D_Save(dir_result + "Roi3D_"+LifName+"_serie"+cell_ID+"_"+staining[channel]+".zip");//Save the 3D ROI
							close("Channel_"+staining[channel]);
							
							
//--------------------------Make granul quantification database					
							open(dir_result + "Q_"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv");
							//Rename the table to "Result" to add measures 
							Table.rename("Q_"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv", "Results");
							selectWindow("Results");
							n_granul = Table.size;
							...
							if (n_granuls==0){
								...
								Table.rename("Results", "Q_"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification");
								saveAs("Results", dir_result+"Q_"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv");
								close("Q_"+LifName+"_serie"+cell_ID+staining[channel]+"_Quantification.csv");
...
//--------------------------Assess all detected granuls for COLOCALIZATION  
					...
					if (n_count == false) {
						print("Skipping colocalization.");
								...
								saveAs("Results", dir_result+"C_"+LifName+"_serie"+cell_ID+"_colocalization.csv");
								close("C_"+LifName+"_serie"+cell_ID+"_colocalization.csv");
								}
